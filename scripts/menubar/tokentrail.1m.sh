...
function appendTrendBlock(lines, menubar) {
  ...
  // Legend rows are direct project links — the menubar is glance + jump.
  // Per-project isolation lives on the dashboard trend chart (hover a
  // legend row there), where in-place re-rendering is actually possible;
  // flyout submenus tried here in 0.2.10 added a hover step for little
  // gain and made the rows unclickable.
